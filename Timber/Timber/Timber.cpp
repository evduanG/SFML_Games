...
int main()
{
	...
	RenderWindow window(vm, "Timber!!!");	// Create and open a window for the game
	...
	Texture textureTree;						// Make a tree sprite
	...
	spriteCloud1.setPosition(0, 0);				// Position the clouds on the left of the screen 
	...
	Font font; font.loadFromFile("fonts/KOMIKAP_.ttf"); 
	...
	while (window.isOpen())
	{
		...
		if (!paused) {
			...
			if (!cloud1Active) {
				...
				if (spriteCloud1.getPosition().x > 1920) {         // Set it up ready to be a whole new cloud next frame   
					cloud1Active = false;
				}
			}
